...
$link = "/Dashboard/Compra/{FacID}";
$ticket ="<a class='btn btn-primary box' href='/Dashboard/Imprimir-Compra/{FacID}'>Ticket</a>";
$GRID->campos(Compras::grid(array("editar" => $edit, "link" => $link,"options" => "'_blank'","status" => $estado,
    "usuarios"=>$usuarios,"imprimir"=>"$ticket")));
$GRID->encabezado("Compras", "FacID", array(false,true,false,true,true), "desc", 500);
$GRID->sql("select distinct c.*, FacID as id, ProvRazon from compras c 
    inner join proveedores on ProvID = FacEmisor where $datos->where group by FacID ", "compras");
$compras = $GRID->mostrar();

function enviar($data)
{
    return "<a class='box btn btn-warning small' href='/Dashboard/Pdf-Compras/".$data["FacID"]."' title='Imprimir'><i class='fas fa-file-pdf'></i></a>";
    /*    
    $e = $data["FacEstado"];
    if ($e == 1)
    {
	...
	$(document).ajaxComplete(function( event,request, settings )
	  {
	  	if(!add_button)
	  	{
  		    ...
  		    {  		       
  		        ...
  		        {
					window.open("/Dashboard/Compra","_self");
